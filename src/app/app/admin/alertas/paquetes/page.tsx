...
import { ListaPaquetesArticulosProps } from "@/types/admin/paquetesArticulos";

export default function AdminArticulosPage() {
  const { addToast } = useToast();
  const [paquetes, setPaquetes] = useState<ListaPaquetesArticulosProps[]>([]);
  ...
  async function EliminarPaqueteArticulos(id: string) {
    const resultado = await eliminarPaqueteArticulo(id);
    ...
    if (resultado.error) {
      ...
      addToast({
        ...
        description: "El paquete de articulos ha sido eliminado exitosamente",
        type: "success",
      });
